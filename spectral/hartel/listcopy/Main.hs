...
data 
    ...
    f_var_cmp a_vx a_vy=
        let { 
            f_char_cmp a_x a_y=((-) :: (Int -> Int -> Int)) (ord a_x) (ord a_y)
    ...
    f_benchmark_main a_n=f_copy (f_sp (f_tp (f_concat (f_take a_n (f_repeat c_prg)))));
    f_copy []=[];
    f_copy (a_x:a_xs)=(:) a_x (f_copy a_xs);
    f_nlines [] a_n=(++) (strict_show_i a_n) "\n";
    f_nlines ('\o012':a_xs) a_n=f_nlines a_xs (((+) :: (Int -> Int -> Int)) a_n (1 :: Int));
    f_nlines (a_x:a_xs) a_n=f_nlines a_xs a_n;
    c_cmp_less::Int;
    ...
    f_cmp_c a_x a_y=((-) :: (Int -> Int -> Int)) (ord a_x) (ord a_y);
